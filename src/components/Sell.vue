...
<section class="form">
    ...
    <div v-if="token" class="all-content">
        <div class="left-side">
            <h3>{{data.title}}</h3>
            <p><b>{{data.object}}</b></p>
            <p><i>Dina aktier:</i> {{data.bought_amount}} st</p>
            <p><i>Senaste priset:</i> {{data.current_price}} kr</p>
            <form @submit.prevent="submitSell($data)">
                <label>Antal</label>
            ...
            <div class="all-messages">
                ...
                <div class="small-screen">
                    <div class="all-objects">
                        ...
                        <i>Antal:</i><br> {{data.amount}} st <br>
...
export default {
    ...
    methods: {
        getMe() {
            let that = this;
            fetch("http://localhost:1337/trade/sell/" + this.object, {
                headers: {
        ...
        submitSell: function () {
            var payload = {
                sell_amount: event.target.amount.value,
                object_amount: this.data.amount,
                bought_amount: this.data.bought_amount,
                price: event.target.price.value,
                cash: this.data.cash,
                user_id: this.user_id
            }
            axios
              .post('http://localhost:1337/trade/sell/' + this.object, payload, {
                  headers: {
